...
void setup() { 
  ...
  servo1.write(0,255,false); // set the intial position of the servo, as fast as possible, run in background
  servo2.attach(servoPin2);  // attaches the servo on pin 9 to the servo object
  servo2.write(0,255,true);  // set the intial position of the servo, as fast as possible, wait until done
} 

void loop() {
  servo1.write(180,127,false);        // move the servo to 180, fast speed, run background
                                        // write(degrees 0-180, speed 1-255, wait to complete true-false)
  servo2.write(180,127,true);         // move the servo to 180, fast speed, wait until done
  
  servo1.write(0,30,false);           // move the servo to 180, slow speed, run in background
  servo2.write(0,30,true);            // move the servo to 180, slow speed, wait until done
} 
